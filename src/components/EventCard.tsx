...
const EventCard = ({ image, title, venue, day, month, published, closed, onTogglePublish }: EventCardProps) => (
  <div className="event-card">
    <div className="relative">
      <img src={image} alt={title} className="w-full h-48 object-cover" loading="lazy" width={640} height={512} />
    ...
    <div className="p-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <button className="px-3 py-1.5 text-xs font-medium rounded-md bg-secondary text-secondary-foreground hover:bg-secondary/80 transition-colors flex items-center gap-1">
          <Edit className="w-3 h-3" /> Edit
        </button>
        <button className="px-3 py-1.5 text-xs font-medium rounded-md bg-destructive text-destructive-foreground hover:bg-destructive/90 transition-colors flex items-center gap-1">
