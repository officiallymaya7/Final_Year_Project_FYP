...
import {
  CalendarDays, Users, ArrowLeft, Loader2, LayoutGrid,
  Sparkles, Trash2, AlertTriangle, Clock, TrendingUp
} from "lucide-react";
...
const Index = () => {
  ...
  const handleDeleteEvent = async () => {
    ...
    try {
      ...
      await supabase.from('event_days').delete().eq('event_id', deleteTarget.id);
  ...
  // ✅ Upcoming Events — aaj ke baad wale
  const today = new Date().toISOString().split("T")[0];
  const upcomingEvents = events
    .filter(e => e.startDate && e.startDate >= today)
    .slice(0, 3);

  // ✅ Recent Events — recently updated/created (top 5)
  const recentEvents = events.slice(0, 5);

  // ✅ Stats — center card ab upcoming dikhayega
  const stats = [
    {
      label: "Total Events",
      value: events.length,
    ...
    },
    {
      ...
      // upcoming events ka preview bhi dikhao
      preview: upcomingEvents.map(e => e.name).join(", ") || "No upcoming events",
    },
    {
      label: "Participants",
  ...
  return (
    <div className="flex h-screen bg-background text-foreground">
      {/* ✅ Sidebar — events aur click handler pass kiya */}
      <DashboardSidebar
        activeType={activeType}
        onTypeChange={setActiveType}
        onCreateEvent={() =>
          activeType === "tech" ? setShowCreate(true) : setShowCustomCreate(true)
        }
        events={events.map(e => ({
          id: e.id,
          name: e.name,
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {view === "overview" ? (
            <div className="max-w-6xl mx-auto space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {stats.map((s) => (
                  ...
                  >
                    ...
                    {/* ✅ Upcoming card mein preview */}
                    {s.preview && (
                      <p className="text-xs text-muted-foreground mt-2 truncate" title={s.preview}>
                        {s.preview}
                      </p>
                    )}
                  </div>
              ...
              {/* ✅ My Events — Recent Activity Style */}
                ...
                {recentEvents.length > 0 ? (
                  <div className="space-y-2">
                    {recentEvents.map((event) => (
                      <div
                        key={event.id}
                        className="group flex items-center justify-between p-4 border border-border rounded-xl bg-card hover:border-primary/50 transition-all"
                      >
                        {/* Left — event info */}
                        <button
                        ...
                        >
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 mb-0.5">
                              ...
                              <span className="font-semibold text-sm group-hover:text-[#F9BB1E] transition-colors truncate">
                                {event.name}
                              </span>
                            </div>
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all opacity-0 group-hover:opacity-100 shrink-0"
                          title="Delete Event"
                        >
                    ...
                    {/* All events — agar 5 se zyada hain */}
                    {events.length > 5 && (
                      <p className="text-xs text-center text-muted-foreground pt-1">
                        + {events.length - 5} more events. Use sidebar to browse all.
                      </p>
                    )}
                  </div>
                ) : (
